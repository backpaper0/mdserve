...
### Static Assets
**Location**: `assets/`
**Purpose**: バイナリに埋め込まれる静的ファイル群。`embed.go` で `//go:embed` ディレクティブにより埋め込み
**CSSレイヤー構成**（順に読み込み）:
1. `github-markdown.css` - GitHubスタイルのMarkdownベーススタイル
2. `highlight.css` - Chromaシンタックスハイライトテーマ
3. `kawaii.css` - パステル調かわいいUIテーマ（ブランディング層）
